...
export function useTradeJournal() {
  ...
  const fetchJournal = useCallback(async (opts?: { symbol?: string; days?: number }) => {
    setLoading(true);
    try {
      const result = await window.electronAPI.getTradeJournal(opts);
      setEntries(result);
    } finally {
      setLoading(false);
    }
  ...
  return { entries, loading, fetchJournal };
}
